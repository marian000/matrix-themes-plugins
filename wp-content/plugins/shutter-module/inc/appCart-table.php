...
$appcart_id = $attributes['appcart_id'];
...
//echo 'appcart_id: ' . $appcart_id;

$product_ids = get_post_meta($appcart_id, 'attached_product_ids', true);
$products_images = get_post_meta($appcart_id, 'products_images', true);
//echo '<pre>';
//print_r($products_images);
//echo '</pre>';

// Asumăm că $products_images conține array-uri cu URL-uri de imagini
if (!empty($products_images)) {
	// Numărul de coloane per rând în Bootstrap 3
	$columns = 3;
	$count = 0;

	echo '<div class="row">';

	foreach ($products_images as $imageGroup) {
		foreach ($imageGroup as $imageUrl) {
			echo '<div class="col-sm-4">';
			echo '<a href="' . esc_url($imageUrl) . '" class="thumbnail" style="width:200px;100px;" target="_blank">';
			echo '<img src="' . esc_url($imageUrl) . '" alt="product image" class="img-responsive">';
			echo '</a>';
			echo '</div>';

			$count++;
			// Dacă am afișat deja 3 imagini, închidem rândul și deschidem unul nou
			if ($count % $columns === 0) {
...
if (empty($product_ids) || !is_array($product_ids)) {
	'<p>No products attached to this AppCart.</p>';
}

$items = array();

// Loop through the $items array to extract product information
foreach ($product_ids as $product_id) {
	...
	if (is_numeric($product_id) && $product_id > 0) {
		...
		if ($product) {
			// Extract necessary information from the product object
			...
			// Display or further process the product information (e.g., create a table or list)
//			echo "Product ID: $product_id, Name: $product_name, Price: $product_price, URL: $product_url <br>";
		} else {
...
?>
<table id="<?php
echo $table_id; ?>" style="width:100%" class="table-app-cart table-bordered <?php
echo $table_class; ?>">
    <thead>
    <tr>
        <th>Nr.</th>
		...
		if ($product_id == 337 || $product_id == 72951) { ?>
			...
			echo $product_id; ?>">
                ...
                <td><?php
					echo $fob_components ? '$' : '£'; ?>
					<?php
					$total_price_quote = get_post_meta($product_id, 'total', true);
					if (!current_user_can('china_admin') && $view_price || current_user_can('administrator')) {
						...
						$formatted_total = number_format($total_price_quote, 2);  // Then format the result
...
//   if (($property_category === 'Shutter & Blackout Blind') || ($pieces[0] == 'Shutter & Blackout Blind') || ($property_category === 'prod2') || ($pieces[0] == 'prod2')
                ...
                <!--                           href="/prod1-all/?appcart_id=--><?php
				//						   echo $appcart_id * 1498765 * 33; ?><!--&id=--><?php
				...
				//							?>
                <!--                        <a class="btn btn-primary transparent" target="_blank"-->
                <!--                           href="/product5-admin/?appcart_id=--><?php
				//						   echo $appcart_id * 1498765 * 33; ?><!--&id=--><?php
				//						   echo $product_id * 1498765 * 33; ?><!--&cust_id=--><?php
				//						   echo $user_id_customer; ?><!--&item_id=--><?php
				//						   echo $item_id; ?><!--">-->
					...
					echo $product_id; ?>">
                        ...
                        <td>Hinge Colour:<strong>
							...
							if ($property_locks == 'Yes' || $property_central_lock == "Yes") { ?> Locks:
                                ...
                                <strong>
									<?php
									$lock_position = ($property_locks == "Yes") ? "Top & Bottom Locks" : "Central Lock";
									echo $lock_position; ?></strong>
                                <br>
                                <!--                Louver Lock:-->
                                <!--                <strong>-->
                                <!--									--><?php
//									echo $property_louver_lock; ?><!--</strong>-->
                            ...
                            Notes:<strong>
								...
								echo $comments_customer; ?></strong>
                            <<?php
							$pictures = get_post_meta($product_id, 'pictures', true);
							if (!empty($pictures)) {
								echo '<br><span>Images:</span>';
								// Wrapper-ul de imagini va sta pe același rând cu span-ul
								echo '<div style="display: inline-block; margin-left: 10px;">';

								$img_index = 1;
								foreach ($pictures as $imageUrl) {
									// Fiecare 'imagine' e afișată într-un container cu display:inline-block;
									echo '<div style="display: inline-block; margin-right: 15px;">';
									echo '<a href="' . esc_url($imageUrl) . '" target="_blank">';
									echo 'Image-' . $img_index;
									echo '</a>';
									echo '</div>';
									$img_index++;
								}
...
//   if (($property_category === 'Shutter & Blackout Blind') || ($pieces[0] == 'Shutter & Blackout Blind') || ($property_category === 'prod2') || ($pieces[0] == 'prod2')
						...
						//									?>
                        <!--                                <a class="btn btn-primary transparent" target="_blank"-->
                        <!--                                   href="/prod-individual/?appcart_id=--><?php
						//								   echo $appcart_id * 1498765 * 33; ?><!--&id=--><?php
						//								   echo $product_id * 1498765 * 33; ?><!--&cust_id=--><?php
						//								   echo $user_id_customer; ?><!--&item_id=--><?php
						//								   echo $item_id; ?><!--">Edit</a>--><?php
						//								} elseif ($property_category === 'Shutter & Blackout Blind' || ($pieces[0] == 'Shutter & Blackout Blind')) {
						//									?>
                        <!--                                <a class="btn btn-primary transparent" target="_blank"-->
                        <!--                                   href="/prod2-all/?id=--><?php
				...
				echo $product_id; ?>">
                    ...
                    <td>Hinge Colour:<strong>
                        ...
                        <br> Layout code:<strong style="text-transform:uppercase;">
							...
							?></strong> <br>
						<?php if ($property_category === 'Shutter & Blackout Blind') { ?>
                            Motorised:<strong style="text-transform:uppercase;">
								<?php
								$property_motorized = get_post_meta($product_id, 'property_motorized', true);
								echo $property_motorized;
								?></strong> <br>
							<?php if ($property_motorized == 'Yes') { ?>
                                Number of motors:<strong style="text-transform:uppercase;">
									<?php
									echo get_post_meta($product_id, 'property_nrMotors', true);
									?></strong>
                                <br> Number of remotes:<strong style="text-transform:uppercase;">
									<?php
									echo get_post_meta($product_id, 'property_nrRemotes', true);
									?></strong><br>
							<?php }
						}
...
<script>
    ...
    jQuery('.btn.btn-danger.delete-item').click(function (e) {
        ...
        var appcart_id = <?php echo json_encode($appcart_id); ?>;
        console.log(item_id);
