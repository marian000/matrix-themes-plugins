...
jQuery('#add-product-single-form .btn-success').on('click', function (e) {
    ...
    if (!nowarranty_checked) {
        ...
        // Citim valorile din input (material & louvre size) și le convertim în numere
        var id_material = parseInt($("input#property_material").val(), 10);
        var check_louvresize = parseInt($("input#property_bladesize").val(), 10);

// Valorile implicite pentru fiecare panou
        var panel1_width = 750;
        var panel2_width = 550;
        var panel3_width = 550;

        /**
         * Tabel de configurare (lookup) pentru panel1, panel2, panel3,
         * bazat pe ID-ul materialului și louvre size.
         *
         * Materiale incluse:
         *  - (188) Ecow
         *  - (5) Ecow Plus
         ...
         *  - (165) -> 114.3 mm
         *
         * Valorile pentru Biow(6) și Biowood Plus(138) sunt ipotetice (0).
         */
        const panelWidthMap = {
            188: { // Ecow
                panel1: {
                    // Conform tabel: (53)->750, (54)->800, (55)->800, (165)->800
                    // (164) nu este specificat, îl poți lăsa neacoperit sau setat la un default
                    overrides: {
                        53: 750,
                        54: 800,
                        55: 850,
                        165: 850
                        // 164: 0 // Dacă ai nevoie și pentru 50.8 mm
                    },
                    default: 750 // fallback dacă louvre size nu e în overrides
                },
                panel2: 550,  // 'all'
                panel3: 550   // 'all'
            },
            ...
            138: { // Biowood Plus
                panel1: {
                    // La fel, datele lipsesc în imagine. Înlocuiește 0 cu valorile reale.
                    overrides: {
                        53: 750,
                        54: 800,
                        55: 890,
            ...
            187: { // Earth
                ...
                panel2: 500, // Poți ajusta dacă tabelul indică altceva
                panel3: 550
            }
        };

// Funcție ajutătoare: calculează lățimea pentru un panel (1,2,3) pe baza mapării
        function getPanelWidth(materialId, louvreSize, panelKey) {
            // Verifică dacă există date pentru materialul respectiv
            if (!panelWidthMap[materialId]) {
                return null; // Sau o valoare default, ex. 750
            }

            const data = panelWidthMap[materialId][panelKey];
            if (!data) {
                return null;
            }

            // Dacă panelKey e "panel1" și există overrides
            if (panelKey === 'panel1') {
                const overrides = data.overrides || {};
                // Dacă avem un override pentru louvreSize
                if (overrides[louvreSize] !== undefined) {
                    return overrides[louvreSize];
                }
                // Altfel, returnează default-ul panel1
                return data.default !== undefined ? data.default : 750;
            } else {
                // panel2, panel3 => doar valori simple
                return data;
            }
        }

// Setăm valorile efective pentru panel1, panel2, panel3
        let possiblePanel1 = getPanelWidth(id_material, check_louvresize, 'panel1');
        if (possiblePanel1 !== null) {
            panel1_width = possiblePanel1;
        }

        let possiblePanel2 = getPanelWidth(id_material, check_louvresize, 'panel2');
        if (possiblePanel2 !== null) {
            panel2_width = possiblePanel2;
        }

        let possiblePanel3 = getPanelWidth(id_material, check_louvresize, 'panel3');
        if (possiblePanel3 !== null) {
            panel3_width = possiblePanel3;
        }
...
// Debug info
        ...
        if (arrayPanels.multiPanels > 0) {
            ...
            var multipanel_max_width_limit = panel3_width;

            current_max_width = arrayPanels.panels1 * multipanel_max_width_limit;
        ...
        if (style_check.indexOf('Tracked') == -1) {
            ...
            if (shutter_type == 'Blackout' && width_shutter > 1400) {
...
// Debug info
        ...
        if (style_check.indexOf('Tracked') == -1) {
            ...
            if (shutter_type == 'Blackout' && width_shutter > 1400 && errors > 0) {
                error_text = error_text + '<br/>Maximum single BoB width is 1400mm';
                addError("property_layoutcode", error_text);
                modalShowError(error_text);
            }
