...
(function ($) {
    $(function () {
            ...
            //fields whose value must stay valid for the selected material
            var SM_FILTER_READY = false;
            var SM_GUARDED_FIELDS = ['property_shuttercolour'];

            //check if a value is still present in a filtered list of property values
            function smValueInList(val, values) {
                if (val === '' || val === null || typeof val === 'undefined') return true;
                for (var i = 0; i < values.length; i++) {
                    if (String(values[i].id) === String(val)) return true;
                }
                return false;
            }

            //a guarded field only counts when it is present and visible on the page
            function smIsFieldInUse(id) {
                //presence only: a collapsed accordion panel makes the field ":hidden"
                //even though it is part of the configuration and gets submitted
                return $('#' + id).length > 0;
            }

            //check a guarded field against the currently selected material
            function smIsGuardedFieldValid(id) {
                var material_val = $('#property_material').val();
                if (!material_val) return true;

                var material_field_id = getPropertyIdByCode('property_material');
                var field_id = getPropertyIdByCode(id);
                var allowed = getRelatedFieldData(field_id, material_field_id, material_val);

                return smValueInList($('#' + id).val(), allowed);
            }

            function smMarkInvalid(id, msg) {
                $('span.error-text.sm-mismatch[data-for="' + id + '"]').remove();
                var $el = $('#' + id);
                var $c = $el.prev('.select2-container');
                //flag survives the value being cleared, so the submit guard still blocks
                $el.attr('data-sm-invalid', '1');
                if ($.fn.collapse) $el.closest('.panel').find('.panel-collapse').collapse('show');
                ($c.length ? $c : $el).addClass('error-field');
                $('<span class="error-text sm-mismatch" data-for="' + id + '">' + msg + '</span>')
                    .insertAfter($c.length ? $c : $el);
            }

            function smClearInvalid(id) {
                $('span.error-text.sm-mismatch[data-for="' + id + '"]').remove();
                $('#' + id).removeAttr('data-sm-invalid')
                    .removeClass('error-field').prev('.select2-container').removeClass('error-field');
            }

            function loadItems(property_code, values) {
                var $el = $('#' + property_code);
                var guarded = SM_FILTER_READY && SM_GUARDED_FIELDS.indexOf(property_code) > -1;
                var wasValid = guarded ? smValueInList($el.val(), values) : true;
                ...
                if (wasValid) {
                    smClearInvalid(property_code);
                } else {
                    //select2 v3 keeps the old value when it is missing from the new list
                    $el.select2('val', '');
                    smMarkInvalid(property_code, 'Not available for the selected Material — please choose again.');
                }
            }
            ...
            function hideTpostBtFrameType() {
                ...
                cusid_ele.forEach(function (element) {
                    if (element.offsetParent !== null) {
                        ...
                        for (var i = 0; i < cusid_ele.length; ++i) {
                            ...
                            if (itemAttr.includes("P7") === true && titleFrameType.includes("P4") === true) {
                                ...
                                $(item).parent().css("display", "block");
                            } else if (itemAttr.includes("7001") === true && titleFrameType.includes("P4") === true) {
                                // exception for 7001 on biowood with frame with P4
                                $(item).parent().css("display", "block");
                            } else {
                        ...
                        // Element is hidden (display: none or not in the DOM tree)
            ...
            //enable the material/colour guard only after the whole synchronous init is done,
            //so saved values are not wiped while the page is being built in edit mode
            setTimeout(function () {
                SM_FILTER_READY = true;
            }, 0);

            //block submit when a guarded field does not match the selected material.
            //capture phase: runs before the jQuery handlers bound in custom-scripts.js
            var sm_form = document.getElementById('add-product-single-form');
            if (sm_form) {
                sm_form.addEventListener('click', function (ev) {
                    var $btn = $(ev.target).closest('.update-btn, .update-btn-admin, .btn-success');
                    if (!$btn.length) return;

                    for (var i = 0; i < SM_GUARDED_FIELDS.length; i++) {
                        var fid = SM_GUARDED_FIELDS[i];
                        if (!smIsFieldInUse(fid)) continue;
                        if ($('#' + fid).attr('data-sm-invalid') === '1' || !smIsGuardedFieldValid(fid)) {
                            ev.preventDefault();
                            ev.stopPropagation();
                            smMarkInvalid(fid, 'Not available for the selected Material.');
                            showErrorModal(
                                'Shutter Colour',
                                'Selected Shutter Colour is not available for the chosen Material. Please select a valid colour.'
                            );
                            return;
                        }
                    }
            ...
            $("#property_shuttercolour").change(function () {
                ...
                if (smIsGuardedFieldValid('property_shuttercolour')) {
                    smClearInvalid('property_shuttercolour');
                }
            });
            ...
            $("#property_material").change(function () {
                ...
                if ($("#property_material").select2('data')) {
                    ...
                    if (product_title_check.indexOf('Green') > -1 || product_title_check.indexOf('EcowoodPlus') > -1) {
            ...
            $('.property-select').on('change', function () {
                ...
                if ($("#property_material").select2('data')) {
                    ...
                    if (product_title_check.indexOf('Biowood') > -1) {
                        // Top&Bottom Locks
                        $("#locks #property_locks ~ span.lbl").text("Top & Bottom Locks");
                    }
                    if (product_title_check.indexOf('Biowood Plus') > -1) {
                        $("#locks #property_locks ~ span.lbl").text("Top & Bottom Locks");
                    }
                    ...
                    if (product_title_check.indexOf('Earth') > -1) {  //teo_02-Earth Hidden Only
                        ...
                        $("#doubleClosingLouvres, #locks").hide();
                        $("#central_lock_position").show();
                        $("#locks").show();
                ...
                if ($('input[name=property_style]:checked').length > 0) {
                    ...
                    if (style_check.indexOf('Shaped') > -1 || style_check.indexOf('French') > -1) {
                        ...
                        if (style_check.indexOf('Solid') > -1) {
                            $('#solidtype input[value="raised"]').click();
                            $('#solidtype input[value="raised"]').attr("checked", "checked");
                            $('#solidtype #property_solidtype_flat').css("display", "none");
                        }
                    ...
                    if ($("#property_material").val() == '187') {
                        if (style_check.indexOf('ALU Panel Only') > -1) {
                            $('#choose-frametype').hide();
                            $('.row.frames').hide();
                        } else {
                            $('#choose-frametype').show();
                            $('.row.frames').show();
                        }
                        if (style_check.indexOf('ALU Fixed Shutter') > -1) {
                            $('input[name="property_frametype"][value="291"]').parent().show();
                            $('input[name="property_frametype"][value="300"]').parent().hide();
                            $('input[name="property_frametype"][value="301"]').parent().hide();
                            $('input[name="property_frametype"][value="302"]').parent().hide();
                        } else {
                            $('input[name="property_frametype"][value="291"]').parent().hide();
                            $('input[name="property_frametype"][value="300"]').parent().show();
                            $('input[name="property_frametype"][value="301"]').parent().show();
                            $('input[name="property_frametype"][value="302"]').parent().show();
                        }
                    }
            ...
            $("#property_material").click(function () {
                ...
                if ($("#property_material").val() == '138' || $("#property_material").val() == '5' || $("#property_material").val() == '137' || $("#property_material").val() == '188') {
            ...
            $("#choose-frametype label").click(function () {
                ...
                if ($('input[name="property_frametype"]:checked').length > 0 || $('input[name="property_frametype"][checked]').length > 0) {
                    ...
                    if (value == 144) { //bottom m-track
                        ...
                        $("#property_frameright").select2("val", '75');
                        $("#property_frametop").select2("val", '80');
                    } else if (value == 143) { //track in board
            ...
            $("#property_locks").change(function () {
                if ($("#property_locks").select2('data')) {
                    var property_locks = $("#property_locks").val();
                    if (property_locks == "Yes") {
                        $('#locks > div:nth-child(2), #locks > div:nth-child(3)').show();
                        // hide position locks empty value
                        $('#section_lock_position select > option:first-child').remove();
                        $('#section_lock_position select').select2().val('Central Lock').trigger('change');
                        $('#section_lock_position').hide();

                    } else if (property_locks == "No") {
                        $('#locks > div:nth-child(2), #locks > div:nth-child(3)').hide();
                        // add position locks default empty option
                        $('#section_lock_position select').prepend('<option value="">Please Select</option>');
                        $('#section_lock_position select').select2().val('').trigger('change');
                        $('#section_lock_position').hide();
            ...
            $("#property_controltype").change(function () {
                if ($("#property_controltype").select2('data')) {
                    var controltype_check = $("#property_controltype").select2('data').value;
            ...
            $("#property_material, #attachment").change(function () {
                ...
                // Show/hide stile-img divs based on selected material
                if ($("#property_material").select2('data')) {
                    var materialId = $("#property_material").select2('data').id;
                    showStileImgForMaterial(materialId);
                }
            });
            ...
            $("#property_layoutcode").on('keypress', function (event) {
                ...
                // Get the selected material value from the dropdown
                let materialValue = $("#property_material").val();

                // Definim caracterele separatoare între grupuri (T, C, G, B)
                let separatorChars = ['T', 'C', 'G', 'B'];

                // Initialize counts for L and R
                let currentRCount = 0;
                let currentLCount = 0;
                let valid = true;  // Flag to track if the input is valid

                // Iterate through each character in the input text
                for (let i = 0; i < text.length; i++) {
                    let char = text[i];

                    // Reset the count when we hit a separator (T, C, G, B)
                    ...
                    // Check if the current character is R or L
                    if (char === 'R') {
                        currentRCount++;
                        currentLCount = 0;  // Reset L count when encountering an R
                    } else if (char === 'L') {
                        currentLCount++;
                        currentRCount = 0;  // Reset R count when encountering an L
                    }

                    // Condition for Biowood Plus (138) — allow up to 3 R or L
                    if (materialValue == '138' || materialValue == '6') {
                        if (currentRCount > 3 || currentLCount > 3) {
                            console.log('Too many R or L characters for Biowood Plus.');
                            valid = false;
                            break;
                        }
                    } else {  // Other materials — allow only up to 2 R or L
                        if (currentRCount > 2 || currentLCount > 2) {
                            console.log('Too many R or L characters for other materials.');
                            valid = false;
                            break;
                        }
                    }
                ...
                // Stop input if the input is invalid
                if (!valid) {
                    return false;  // Prevent further input if invalid
                }
        ...
        Tracked Layout Code
         */
            $("#property_layoutcode_tracked").on('keyup', function () {
                // console.log('keyup press');
                var text = $(this).val().toUpperCase();
                //updateLayoutFields(text);
            });
            ...
            $("#property_layoutcode_tracked").on('keypress', function (event) {
                ...
                if (style_check.indexOf('Tracked By-Fold') > -1) {
                    // if free folding is yes then allow to press F and / on layout (must be even)
                    // const freeloadingVal = $('input[name="property_freefolding"]').val();
                    const freeloadingVal = $('input[name="property_freefolding"]:checked').val();
                    console.log('freeloadingVal', freeloadingVal);
                    if (freeloadingVal === "yes") {
                        return "F" == character || "/" == character || event.keyCode == 8;
                    } else return "R" == character || "L" == character || event.keyCode == 8;
                }

                if (style_check.indexOf('Tracked By-Pass') > -1) {
                    if ("B" == character || "F" == character || event.keyCode == 8) {
                        return true;
                    } else {
                        return false;
                    }
            ...
            let timer;
            $("input#property_layoutcode_tracked").keyup(function () {
                const freeloadingVal = $('input[name="property_freefolding"]:checked').val();
                if (freeloadingVal === "yes") {
                    ...
                    timer = setTimeout(function () { // assign timer a new timeout
                        ...
                        if (count <= 12) {
                            ...
                            console.log("Error by fold layout");
                            addError("property_layoutcode_tracked", 'Layout Configuration not supported. Please correct.');
                        }
            ...
            /**
             * Show/hide stile-img divs based on selected material
             * Maps material IDs to div names for stile images
             */
            function showStileImgForMaterial(materialId) {
                // Convert materialId to integer for consistent comparison
                var matId = parseInt(materialId, 10);

                // Material ID to div name mapping
                var materialDivMap = {
                    187: 'earth',       // Earth
                    188: 'ecowood',     // Ecowood
                    139: 'basswoodPlus', // Basswood Plus
                    147: 'basswood',     // Basswood
                    6: 'biowood',       // Biowood
                    138: 'biowoodPlus', // BiowoodPlus
                    137: 'green',       // Green
                    5: 'ecowoodPlus'    // EcowoodPlus
                };

                // Hide all stile-img divs first
                jQuery('[id^="stile-img-"]').hide();

                // Show the correct div for selected material
                var divName = materialDivMap[matId];
                if (divName) {
                    jQuery('#stile-img-' + divName).show();
                    console.log('showStileImgForMaterial: showing stile-img-' + divName + ' for material ' + matId);
                } else {
                    console.log('showStileImgForMaterial: no mapping found for material ' + matId + ' (original: ' + materialId + ')');
                }
            }
                ...
                for (var i = 0; i < text.length; i++) {
                    ...
                    if (text.charAt(i).toUpperCase() == 'T') {
                        ...
                        if (tchar_nr < 2) {
                            ...
                            tchar_nr++;
                            console.log('tchar_nr after : ' + tchar_nr);
                            ...
                            if (material_id == 187) {
                                ...
                                console.log('jQuery(\'.type-img-earth\').show();');
                            }
                            // Basswood Plus 139
                                ...
                                console.log('jQuery(\'.type-img-basswoodPlus\').show();');
                            }
                             // Basswood 147
                                ...
                                console.log('jQuery(\'.type-img-basswood\').show();');
                            }
                            // BiowoodPlus 138
                                ...
                                console.log('jQuery(\'.type-img-biowoodPlus\').show();');
                            }
                            // Biowood 6
                            ...
                            // Green 137
                                ...
                                console.log('jQuery(\'.type-img-green\').show();');
                            ...
                            // ecowood 188
                                ...
                                console.log('jQuery(\'.type-img-ecowood\').show();');
                            }
                    ...
                    if (text.charAt(i).toUpperCase() == 'B') {
                        ...
                        if (property_material == 188) {
                            ...
                            // OLD:  addFieldBuildAngleSelect(labela, id1a, 1);
                            addField(labela, id1a, 1);
                        } else {
            ...
            function addFieldCheckboxBuildout(label, id, helptext) {
                ...
                //create extra column element
                var new_column = '<div class="pull-left extra-column-buildout ' + id + '">' + new_column_contents + '</div>';
                var element = $(new_column);
                $(element).find(".extra-column-label").html(label);
            ...
            function addFieldCheckboxBuildoutSelect(label, id, helptext, character) {
                ...
                $(element).find("input").remove();
                if (character === 'b') {
                    $(element).find(".extra-column-label").append("<select id='buildout-select' name='bay-post-type'><option value='normal'>Normal</option><option value='slim'>Slim</option><option value='flexible'>Flexible</option></select>");
                } else if (character === 't') {
            ...
            function showMidrailPositionCritical() {
                ...
                if ($('input[name=property_style]:checked').length > 0) {
                    style_check = $('input[name=property_style]:checked').data('title');
                    if (style_check.indexOf('Solid Tracked By-Pass') > -1 || style_check.indexOf('Solid Tracked By-Fold') > -1) {
                        $("#midrail-position-critical").hide();
                    }
                }
            }
            ...
            function checkStyleTier() {
                ...
                console.log('checkStyleTier inputValue', inputValue);
                ...
                // Show the element
                $("#property_controltype").closest('div').show().removeClass('solid-selected');
                ...
                if (style_check.indexOf('Special') > -1) {
                    $(".tot-height").fadeIn();
                    if ($("#property_material").select2('data')) {
                        let product_title_check = $("#property_material").val();
                        if (product_title_check.indexOf('Earth') == 187) {
                            $("#property_horizontaltpost").prop('checked', true);
                        }
                    }
                    ...
                    if (style_check.indexOf('Tracked') < 0) {
                    ...
                    if (style_check.indexOf('Shaped') > -1) {
                        ...
                        // if form has attribute edit yes then remove attr checked
                        if ($('form').attr('edit') === 'yes') {
                            $('#solidtype input[value="raised"]').removeAttr("checked");
                        }

                        $('#solidtype #property_solidtype_flat').removeAttr("style");
                    }
                    ...
                    $("#property_controltype").closest('div').hide().addClass('solid-selected');
                ...
                if (style_check.indexOf('Tracked By-Fold') > -1) {
                    ...
                    $("#trackedtype, #free-folding, #property_layoutcode_tracked").show();
                    $("#property_layoutcode, #trackedtyperecess, #tracksnumber, #lightblocks, #bypasstype").hide();
            ...
            function checkShutterType() {
                if (shutter_type == 'Shutter') {
                    ...
                    var disable_property_values = [137, 5, 33, 35];
                }
            ...
            function filterControlType() {
                ...
                $.each(controltype_data, function (index, row) {
                    ...
                    if (property_material.indexOf('Earth') == -1 &&
                        row.value.indexOf('Clearview') == -1 &&
                        row.value.indexOf('Hidden Tilt with Louver Lock') == -1 &&
                        row.value.indexOf('Fixed') == -1) {
                    ...
                    if (property_material.indexOf('Earth') > -1 &&
                        row.value.indexOf('Clearview') == -1 &&
                        row.value.indexOf('Centre') == -1 &&
                        row.value.indexOf('Offset') == -1 &&
                        row.value.indexOf('Concealed') == -1) {
                        if (property_material.indexOf('PVC') > -1 && property_material.indexOf('UK') > -1) {
                            if (row.value.indexOf('Clearview') == -1)
                                return true;
                        }
            ...
            function shutterConfig() {
                ...
                // myShutter to json
                console.log("myShutter to json");
                // console.log(JSON.stringify(myShutter, null, 2));
            ...
            function updateShutter() {
                ...
                if (style.indexOf("Shaped") >= 0) {
                    ...
                    $("#canvas_container1").html("&nbsp;Factory will suply drawings to be confirmed");
                    $("#canvas_container1 svg").remove();
                    $(".print-drawing").hide();
                    return;
                } else {
            ...
            function getFrameTypeCode() {
                ...
                var style_check = $('input[name=property_style]:checked').data('title');
                if (style_check && (style_check.indexOf('ALU Panel Only') > -1 || style_check.indexOf('ALU Fixed Shutter') > -1)) {
                    code = 'L50';
                }
