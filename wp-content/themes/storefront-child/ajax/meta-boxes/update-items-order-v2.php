...
//    wc_get_order($order_id)->add_product($product, $item_data['quantity']);
        ...
        if (!empty(get_post_meta($product_id, '_price', true))) {
            ...
            $item_propertyes_new['total_price'] = get_post_meta($product_id, '_price', true);
        }
        if (get_post_meta($product_id, 'shutter_category', true) == 'Shutter') {
            wc_update_order_item_meta($item_id, 'product_id', 20179);
            $item_propertyes_new['product_id'] = 20179;
        }
        if (get_post_meta($product_id, 'shutter_category', true) == 'Batten') {
            wc_update_order_item_meta($item_id, 'product_id', 20191);
            $item_propertyes_new['product_id'] = 20191;
        }
        if (get_post_meta($product_id, 'shutter_category', true) == 'Shutter & Blackout Blind') {
            wc_update_order_item_meta($item_id, 'product_id', 35190);
            $item_propertyes_new['product_id'] = 35190;
        }
        ...
        foreach ($layout_code as $key => $l) {
            ...
            if ($l == 't' || $l == 'T') {
                ...
                $item_propertyes_new['property_t' . $key] = $property_t;
                $order_t++;
                $i++;
            }
            if ($l == 'b' || $l == 'B') {
                ...
                $item_propertyes_new['property_bp' . $key] = $property_bp;
                ...
                $item_propertyes_new['property_ba' . $key] = $property_ba;
                $order_b++;
                $i++;
            }
            if ($l == 'c' || $l == 'C') {
                ...
                $item_propertyes_new['property_c' . $key] = $property_c;
                $order_c++;
                $i++;
            }
            if ($l == 'g' || $l == 'G') {
        ...
        $item_propertyes_new['quantity'] = $item_data['quantity'];
        wc_update_order_item_meta($item_id, 'quantity', $item_data['quantity']);

        wc_delete_order_item_meta($item_id, 'cart_item_metas');
        if (!empty($item_data['cart_item_metas']['cart_item_metas'])) {
            $item_data['cart_item_metas']['cart_item_metas'] = '';
        }
        $item_propertyes['cart_item_metas'] = $item_propertyes_new;
        wc_update_order_item_meta($item_id, 'cart_item_metas', $item_propertyes_new);
