...
if (!function_exists('matrix_build_repair_email_body')) {
	/**
	 * Build the HTML email body for a repair order notification.
	 *
	 * @param int $post_id ID of the order_repair post.
	 * @return string HTML email body.
	 */
	function matrix_build_repair_email_body($post_id)
	{
		$repair_id          = $post_id;
		...
		// Use the post author as the customer user (set when repair was created).
		$repair_post      = get_post($repair_id);
		$user_id_customer = $repair_post ? (int) $repair_post->post_author : 0;
                        ...
                        <table style="width:100%; display:table; border: 2px solid #333; border-radius: 5px;"
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th>' . get_post_meta($order_id, 'cart_name', true) . '</th>
                                    <th>Order date:</th>
                                ...
                                <tr>
                                    ...
                                    <th>' . get_the_date('Y-m-d', $repair_id) . '</th>
                                </tr>
                            </thead>
                        </table>
                        <br>
		...
		foreach ($items as $item_id => $item_data) {
			...
			if ($product_id == 337 || $product_id == 72951) {
			...
			} elseif (isset($term_list[0]) && $term_list[0]->slug == 'pos') {
				$email_body .= '<tr style="';
				if (isset($description_damage[$item_id]) && trim($description_damage[$item_id]) != '' || isset($remedial_action[$item_id]) && trim($remedial_action[$item_id]) != '') {
					$email_body .= 'background: #cceeff';
				}
				...
				$email_body .= '">
                                    <td>
                                        ' . $property_category . '
                                        <br>Room: <strong>' . $property_room_other . '</strong>
                                        <br>Material: <strong>' . $atributes[$property_material] . '</strong>
                                    </td>
                                    <td></td>
                                    <td>
                                        Shutter Colour:<strong>' . $atributes[$property_shuttercolour] . '</strong>
                                        <br>Notes:<strong>' . $comments_customer . '</strong>
                                    </td>
                                    <td>
                                        <strong>' . number_format((float)$property_volume, 5) . ' q/m </strong>
                                        <br>Qty: <strong>' . $batten_qnt . '</strong>
                                        <br>Width:<strong>' . $property_width . '</strong>
                                        <br>Height:<strong>' . $property_height . '</strong>
                                        <br>Depth:<strong>' . $property_depth . '</strong>
                                    </td>
                                </tr>';
			} else {
				$email_body .= '<tr style="';
				...
				$email_body .= '
                                    <br>Room: <strong>' . $property_room_other . '</strong>
                                    <br>Installation style:<strong>' . $atributes[$property_style] . '</strong>
                                    <br>Shape:';
				if (!empty($attachment)) {
					$email_body .= 'yes';
				} else {
				...
				$email_body .= '
                                    <br>Midrail Height:<strong>' . $property_midrailheight . '</strong>
                                    <br>';
				...
				$email_body .= '
                                    Louvre size:<strong>' . $atributes[$property_bladesize] . '</strong>
                                    <br>Fit: <strong>' . $atributes[$property_fit] . '</strong>
                                </td>
                                <td>
                                    Frame type:<strong>' . $atributes[$property_frametype] . '</strong>
                                    <br>Stile type:<strong>' . $atributes[$property_stile] . '</strong>
                                    <br>Frame Left:<strong>' . $atributes[$property_frameleft] . '</strong>
                                    <br>Frame Right:<strong>' . $atributes[$property_frameright] . '</strong>
                                    <br>Frame Top:<strong>' . $atributes[$property_frametop] . '</strong>
                                    <br>Frame Bottom:<strong>' . $atributes[$property_framebottom] . '</strong>
                                    <br>';
				...
				$email_body .= '
                                ...
                                <td>
                                    Hinge Colour:<strong>' . $atributes[$property_hingecolour] . '</strong>';
				if ($property_hingecolour == 93) {
					$email_body .= ' (+' . get_post_meta(1, 'Stainless_Steel', true) . '%)';
				}
				$email_body .= '
                                    <br>Shutter Colour:<strong>' . $atributes[$property_shuttercolour] . '</strong>
                                    <br>';
				...
				if ($atributes[$property_blackoutblindcolour]) {
					$email_body .= 'Blackout Blind Colour: <strong>' . $atributes[$property_blackoutblindcolour] . '</strong><br>';
				}
				$email_body .= 'Control Type:<strong>' . $atributes[$property_controltype] . '</strong>';
				...
				if (!empty($atributes[$property_controlsplitheight])) {
					$email_body .= '<br>Control Split Height: <strong>' . $atributes[$property_controlsplitheight] . '</strong>';
				}
				$email_body .= '
                <br>Layout code:<strong style="text-transform:uppercase;">' . $property_layoutcode . '</strong>
                <br>';
				...
				if (!empty(get_post_meta($product_id, 'property_t1', true))) {
					if ($property_tposttype) {
						$email_body .= 'T-Post Type: <strong>' . $atributes[$property_tposttype] . '</strong><br>';
					}
				}
				if ($property_sparelouvres == 'Yes') {
					$email_body .= 'Include 2 x Spare Louvre: <strong>' . $property_sparelouvres . '</strong> (+6$)<br>';
				}
				if ($property_ringpull == 'Yes') {
					$email_body .= 'Ring Pull: <strong>' . $property_ringpull . '</strong> (+35$)<br>How many rings?: <strong>' . $property_ringpull_volume . '</strong><br>';
				}
				if ($property_locks == 'Yes') {
					$email_body .= 'Locks: <strong>' . $property_locks . '</strong> (+35£)<br>How many locks?: <strong>' . $property_locks_volume . '</strong><br>';
				}
				$email_body .= 'Notes:<strong>' . $comments_customer . '</strong>
                </td>
                <td>
                    <strong>' . number_format((float)$property_total, 2) . ' sq/m </strong>
                    <br>Qty: <strong>' . $item_data['quantity'] . '</strong>
                    <br>Width:<strong>' . $property_width . '</strong>
                    <br>Height:<strong>' . $property_height . '</strong>
                </td>
                </tr>';
			...
			if (isset($description_damage[$item_id]) && trim($description_damage[$item_id]) != '' || isset($remedial_action[$item_id]) && trim($remedial_action[$item_id]) != '' || isset($attachment_id[$item_id]) && trim($attachment_id[$item_id]) != '') {
				$email_body .= '
                        <tr style="background-color: #cceeff">
                            <td colspan="5">
                                <div>
                                    <label><strong>Description of error/damage</strong></label>
                                    <p>' . $description_damage[$item_id] . '</p>
                                    <label><strong>Remedial action requested</strong></label>
                                    <p>' . $remedial_action[$item_id] . '</p>
                                    <label><strong>Covered by Warranty: <strong>' . $warranty[$item_id] . '</strong></strong></label>
                                    <br>
                                    <label><strong>Images with damage</strong></label>
                                    <div id="shape-upload-container">';
				if (isset($attachment_id[$item_id]) && $attachment_id[$item_id]) {
					foreach ($attachment_id[$item_id] as $img_url) {
						$email_body .= '
                                        <div class="col-lg-4 col-md-6" style="max-width: 50%;float: left;">
                                            <img src="' . $img_url . '" alt="" style="max-height: 200px; width: auto;">
                                            <p><a href="' . $img_url . '">IMG LINK</a></p>
                                        </div>
...
if (!function_exists('matrix_send_repair_notification')) {
	/**
	 * Build and send the repair order notification email.
	 *
	 * @param int $post_id ID of the order_repair post.
	 * @return array ['result' => bool, 'recipients' => array]
	 */
	function matrix_send_repair_notification($post_id)
	{
		...
		$recipients = send_mail_repair($warranty_find_yes);
		...
		$body    = matrix_build_repair_email_body($post_id);
		$headers = array('Content-Type: text/html; charset=UTF-8', 'From: Service LifetimeShutters <service@lifetimeshutters.co.uk>');

		// Collect image files as physical attachments
		$attachments    = array();
		$attachment_ids = get_post_meta($post_id, 'attachment_id_array', true);
		if (is_array($attachment_ids)) {
			foreach ($attachment_ids as $item_images) {
				if (is_array($item_images)) {
					foreach ($item_images as $img_url) {
		...
		return array('result' => (bool) $result, 'recipients' => array_values($recipients));
	}
}


if (!function_exists('matrix_log_repair_email')) {
	/**
	 * Append an entry to the repair email send log stored in post meta.
	 *
	 * @param int   $post_id    ID of the order_repair post.
	 * @param bool  $result     Whether wp_mail succeeded.
	 * @param array $recipients List of recipient email addresses.
	 */
	function matrix_log_repair_email($post_id, $result, $recipients)
	{
		$user_id   = get_current_user_id();
		$user_data = get_userdata($user_id);
		$user_name = $user_data ? $user_data->display_name : 'Unknown';

		$log = get_post_meta($post_id, '_repair_email_log', true);
		if (!is_array($log)) {
			$log = array();
		}
		...
		update_post_meta($post_id, '_repair_email_log', $log);
	}
}


if (!function_exists('matrix_resend_repair_email_ajax')) {
	/**
	 * AJAX handler: resend repair notification email from admin.
	 */
	function matrix_resend_repair_email_ajax()
	{
		check_ajax_referer('matrix_repair_email_nonce', 'nonce');
		...
		$send     = matrix_send_repair_notification($post_id);
		$result   = $send['result'];
		$recipients = $send['recipients'];

		matrix_log_repair_email($post_id, $result, $recipients);

		if ($result) {
			wp_send_json_success(array('message' => 'Email sent successfully.', 'recipients' => $recipients));
		} else {
			wp_send_json_error(array('message' => 'wp_mail() returned false. Check server mail configuration.'));
		}
	}
}

add_action('wp_ajax_matrix_resend_repair_email', 'matrix_resend_repair_email_ajax');
